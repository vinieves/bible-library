   ...
   @class([
    ...
    <div class="flex items-stretch">
        <div class="video-list-thumb">
            ...
            <span class="video-list-thumb-play" aria-hidden="true">
                <span @class([
                    ...
                    'bg-brown/90 text-cream shadow-brown/25' => $hasAccess,
                    'border border-gold/30 bg-brown-deep/60 text-cream/60' => ! $hasAccess,
                ])>
                    @if($hasAccess)
            ...
            @if($hasAccess && $percent >= 100)
                <span class="absolute left-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-brown/90 text-cream">
        ...
        <div class="flex min-w-0 flex-1 flex-col justify-center px-4 py-3.5 sm:px-5 sm:py-4">
            <h3 class="text-base font-medium leading-snug text-ink sm:text-lg">
                {{ $video->title }}
            </h3>

            @if(filled($video->description))
                <p class="mt-1.5 line-clamp-2 text-sm leading-relaxed text-tan sm:text-base">
                    {{ $video->description }}
                </p>
            @endif
