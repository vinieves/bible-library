    ...
    <header class="shrink-0 border-b border-bible-gold/20 bg-bible-dark/95 backdrop-blur">
        <div class="flex min-h-[3.25rem] items-center justify-between gap-2 px-3 py-2 sm:px-4">
            <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('members.dashboard') }}"
               class="inline-flex shrink-0 items-center gap-1 rounded-lg px-2 py-2 text-sm font-medium text-bible-gold transition hover:bg-bible-gold/10 sm:text-base">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
                </svg>
                <span class="hidden sm:inline">Volver</span>
            </a>
            <p class="min-w-0 flex-1 truncate text-center text-sm font-medium text-bible-cream/90 sm:text-base">
                @yield('title')
            </p>
