...
@section('content')
    <div class="rounded-2xl border border-bible-gold/20 bg-bible-dark p-6 text-center sm:p-8">
        ...
        @else
            <div class="mx-auto mb-5 flex h-24 w-24 items-center justify-center rounded-full bg-bible-gold/10 text-4xl">
                🔒
            </div>
        @endif

        <h1 class="text-xl font-semibold text-bible-gold sm:text-2xl">{{ $audioTrack->title }}</h1>

        <p class="mx-auto mt-4 max-w-sm text-base leading-relaxed text-bible-cream/80">
            Este audio forma parte de la {{ $subscriptionTitle }} Premium.
        </p>
        <p class="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-bible-cream/55">
